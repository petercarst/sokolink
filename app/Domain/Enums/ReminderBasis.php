...
/**
 * HOW the due date was estimated, strongest evidence first.
 *
 * Mirrors `reorder_reminders`.`basis`. The two are asserted equal by
 ...
 */
enum ReminderBasis: string
{
    case ObservedInterval = 'observed_interval';
    case SellerHint = 'seller_hint';
    ...
    {
        return match ($this) {
            self::ObservedInterval => "This customer's own repeat interval",
            self::SellerHint => "The seller's guidance for this product",
            self::CategoryDefault => 'A default for this category',
            self::None => 'Nothing to estimate from - send nothing',
        };
    }
}
